...
FROM posts a
INNER JOIN posts_etiquetas b ON a.id = b.post_id
INNER JOIN etiquetas c ON c.id = b.etiqueta_id
GROUP BY a.id;

-- ¿Cuál es el tag que mas se repite?
SELECT  etiquetas.nombre_etiqueta, 
...
SELECT *
FROM etiquetas a
LEFT JOIN posts_etiquetas b on a.id = b.etiqueta_id
WHERE b.etiqueta_id IS NULL;
