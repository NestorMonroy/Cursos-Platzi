...
class CreateRideRatingSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if ride.offered_by == user:
            raise serializers.ValidationError("You can't rate your own ride")
        if ride.departure_date <= timezone.now():
            raise serializers.ValidationError('Ride has not departured')

        if ride.activated == True:
            raise serializers.ValidationError('Ride has not finished')
